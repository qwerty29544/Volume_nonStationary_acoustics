...
def FPI_solver(A_matrix, f_vector, u0_vector=None, eps=10e-7, n_iter=10000):
    ...
    :param eps: Заданная точность для приближаемого вектора решения исходного операторного уравнения,
    по метрике max(abs(u(s) - u(s-1)))
    ...
    max_value = np.max(A_matrix, f_vector)
    # TODO: 2*N*N операций
    ...
    for iter_idx in range(1, (n_iter + 1)):
        ...
        if np.max(np.abs(u_vector - u0_vector)) < eps:
            break
